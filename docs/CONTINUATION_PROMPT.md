...
1. Commit the M06 checkpoint documents and push `codex/m06-prompt-requirements`.
2. Implement durable tenant-scoped change-request and requirement-revision persistence with forward/recovery migration evidence.
3. Add authenticated create/review API routes and accessible intake/review UI, followed by controller-backed budget-denial/no-bypass fixtures. Do not make a live model call before the minimum AI Cost Controller exists.
