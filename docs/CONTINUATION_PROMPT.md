...
M04 is active. Local implementation commit `b15a2c3` adds installation-first GitHub App onboarding with a distinct `repository:connect` permission; hashed/expiring/single-use state; opaque credential references; immutable repository/default-branch/permission synchronization; read-only readiness; typed API and PostgreSQL seams; raw HMAC webhook processing for installation/repository/push events; migration `0003_m04_github_onboarding`; and fixture/runbook evidence. ADR-013 records the decision. No M04 push, PR, or remote CI evidence exists yet; no real GitHub App was configured.
...
1. Commit the M04 checkpoint documents, push `codex/m04-github-onboarding`, and open a draft PR stacked on `codex/m03-provider-framework`.
2. Observe its CI and require full validation plus ephemeral PostgreSQL migration before marking M04 complete.
3. Record the PR/run/job/final commit evidence, push the completion record, confirm its CI, then begin M05 in milestone order.
