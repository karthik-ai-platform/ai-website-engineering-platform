...
1. Build and publish the approved digest-pinned hardened runner image for the selected Vercel Sandbox profile; implement immutable checkout and short-lived credential brokering without injecting production secrets.
...
3. Integrate the adapter through durable worker dispatch and add live forbidden host filesystem/process/network/credential/production-secret, resource-limit, malicious-install-script, cleanup, and artifact-integrity acceptance evidence. Never cite the conformance fixture or mocked session verification as production isolation; do not make a live model call or merge any PR autonomously.
